...
PHP_METHOD(Aerospike, put)
{
    ...
    // Process write policy
    if (z_write_policy) {
        if (zval_to_as_policy_write(z_write_policy, &write_policy, &write_policy_p, &as_ptr->config.policies.write) != AEROSPIKE_OK) {
