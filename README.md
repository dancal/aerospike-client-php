...
### Building for Dockerfile

This Dockerfile builds an image with the Aerospike PHP client installed. It clones the Aerospike PHP client repository, builds the extension, and enables it in the PHP configuration.

	FROM php:8.3-cli
    RUN set -eux mkdir -p /tmp/aerospike-client-php \
        && cd /tmp \
        && git clone https://github.com/dancal/aerospike-client-php.git \
        && cd /tmp/aerospike-client-php/src \
        && phpize \
        && ./build.sh \
        && make install \
        && rm -rf /tmp/aerospike-client-php \
